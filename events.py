...
def binned_events(era, erb, expo, mv, det, fx, g):
    if fx.ty == 'sns':
        return quad(snsrates, era, erb, args = (mv, det, fx, g, ))[0]*expo*JoulePerKg*GeVPerJoule*24*60*60/det.m
...
def ratesm(er, mv, det, fx, g):
    deno = 2*sqrt(2)*gf*(2*det.m*er+mv**2)
    qvs = (0.5*det.z*(0.5-2*ssw+2*g['umm']/deno+g['dmm']/deno)+0.5*det.n*(-0.5+g['umm']/deno+2*g['dmm']/deno))**2 + \
          (0.5*det.z*(2*g['uem']/deno+g['dem']/deno)+0.5*det.n*(g['uem']/deno+2*g['dem']/deno))**2 + \
          (0.5*det.z*(2*g['umt']/deno+g['dmt']/deno)+0.5*det.n*(g['umt']/deno+2*g['dmt']/deno))**2
    return 2/pi*(gf**2)*(2*fx.numfint(er, det.m)-det.m*er*fx.numfinvs(er, det.m))*det.m*qvs*formfsquared(sqrt(2*det.m*er), det.z+det.n)
...
def ratesp(er, mv, det, fx, g):
    ...
    qvs = (0.5*det.z*(0.5-2*ssw+2*g['umm']/deno+g['dmm']/deno)+0.5*det.n*(-0.5+g['umm']/deno+2*g['dmm']/deno))**2 + \
          (0.5*det.z*(2*g['uem']/deno+g['dem']/deno)+0.5*det.n*(g['uem']/deno+2*g['dem']/deno))**2 + \
          (0.5*det.z*(2*g['umt']/deno+g['dmt']/deno)+0.5*det.n*(g['umt']/deno+2*g['dmt']/deno))**2
    return 2/pi*(gf**2)*(2*fx.nupfint(er, det.m)-det.m*er*fx.nupfinvs(er, det.m))*det.m*qvs*formfsquared(sqrt(2*det.m*er), det.z+det.n)

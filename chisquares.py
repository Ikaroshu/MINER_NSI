...
class Chisquare:
    ...
    def __init__(self, det, fx, g, mv, expo, nbins, div='linear'):
        ...
        self.binned_sm = array([binned_events(self.ebin[i], self.ebin[i+1], self.expo, self.mv, self.det, self.fx, couplings()) for i in range(self.ebin.shape[0]-1)])
        self.binned_bg = array([binned_background(self.ebin[i], self.ebin[i+1], self.det, self.expo) for i in range(self.ebin.shape[0]-1)])
        self.binned_nsi = array([binned_events(self.ebin[i], self.ebin[i+1], self.expo, self.mv, self.det, self.fx, self.g) for i in range(self.ebin.shape[0]-1)])
    ...
    def lgl(self, nf, nb, mu, bsm, sm, bg):
        ...
        ni = bsm+sm+bg
        return sum(ni*log(nui)-nui-((nf-1)**2)/(2*(self.fx.flUn**2))-((nb-1)**2)/(2*(self.det.bgUn**2)))     # n! is constant
    ...
    def tmu_binned(self):
        if self.th != self.det.erMin:
            ...
            self.binned_bg = array([binned_background(self.ebin[i], self.ebin[i+1], self.det, self.expo) for i in range(self.ebin.shape[0]-1)])
            self.th = self.det.erMin
        binned_nsi = array([binned_events(self.ebin[i], self.ebin[i+1], self.expo, self.mv, self.det, self.fx, self.g) for i in range(self.ebin.shape[0]-1)])
    ...
    def tmuc(self, coup):
        if self.th != self.det.erMin:
            ...
            self.binned_bg = array([binned_background(self.ebin[i], self.ebin[i+1], self.det, self.expo) for i in range(self.ebin.shape[0]-1)])
            self.th = self.det.erMin
        sm = array([binned_events(self.ebin[i], self.ebin[i+1], self.expo, self.mv, self.det, self.fx, coup) for i in range(self.ebin.shape[0]-1)])
